...
class Blob {
    ...
    WorldImage blobImage() {
        return new FromFileImage(this.center, "small-shark.png").overlayImages(new CircleImage(
    ...
    public Blob moveBlob(String ke) {
        if (ke.equals("right")) {
            return new Blob(new Posn(this.center.x + 5, this.center.y), this.radius, this.col);
        }
        else if (ke.equals("left")) {
            return new Blob(new Posn(this.center.x - 5, this.center.y), this.radius, this.col);
        }
        else if (ke.equals("up")) {
            return new Blob(new Posn(this.center.x, this.center.y - 5), this.radius, this.col);
        }
    ...
    Blob randomMove(int n) {
        return new Blob(new Posn(this.center.x + this.randomInt(n), this.center.y
            + this.randomInt(n)), this.radius, this.col);
    }
...
class BlobWorldFun extends World {
     ...
     * It illustrates the use of mostof the <code>WorldImage</code> shapes
     */
            ...
            new OverlayImages(new EllipseImage(new Posn(12, 12), 25, 25, new Green()), 
                new OverlayImages(new DiskImage(new Posn(100, 150), 10, new Black()),
                    new OverlayImages(new CircleImage(new Posn(100, 150), 10, new White()), 
                        new OverlayImages(new RectangleImage(new Posn(100, 150), 10, 10, 
                                            new White()), 
                            new OverlayImages(new LineImage(new Posn(95, 145), new Posn(105, 155), 
                                                new Red()), 
    ...
    public WorldImage lastImage(String s) {
        return new OverlayImages(this.makeImage(), new TextImage(new Posn(100, 40), s, Color.red));
    }
    ...
    public WorldEnd worldEnds() {
        ...
        if (this.blob.outsideBounds(this.width, this.height)) {
            return new WorldEnd(true, new OverlayImages(this.makeImage(), 
                new TextImage(new Posn(100, 40), "Blob is outside the bounds", Color.red)));
        }
        // time ends is the blob falls into the black hole in the middle
        if (this.blob.nearCenter(this.width, this.height)) {
            return new WorldEnd(true, new OverlayImages(this.makeImage(), 
                new TextImage(new Posn(100, 40), "Black hole ate the blob", 13, 3, Color.red)));
        }
        else {
...
class BlobExamples {
    ...
    BlobWorldFun b1mouse50x50w = new BlobWorldFun(new Blob(new Posn(50, 50), 20, new Red()));

    BlobWorldFun bwOutOfBounds = new BlobWorldFun(new Blob(new Posn(100, 350), 50, new Red()));

    BlobWorldFun bwInTheCenter = new BlobWorldFun(new Blob(new Posn(100, 150), 50, new Red()));

    /** test the method moveBlob in the Blob class */
    boolean testMoveBlob(Tester t) {
        return t.checkExpect(this.b1.moveBlob("left"), this.b1left, "test moveBolb - left " + "\n")
    ...
    boolean testOnKeyEvent(Tester t) {
        return t.checkExpect(this.b1w.onKeyEvent("left"), this.b1leftw, "test moveBolb - left "
            + "\n")
            ...
            t.checkExpect(this.b1Gw.onKeyEvent("x").lastWorld, new WorldEnd(true,
                new OverlayImages(this.b1Gw.makeImage(), new TextImage(new Posn(100, 40),
                    "Goodbye", Color.red))));
    }
    ...
    boolean testOutsideBounds(Tester t) {
        return t.checkExpect(this.b1.outsideBounds(60, 200), true,
            ...
            t.checkExpect(this.b1.outsideBounds(100, 90), true, "test outsideBounds below")
            &&

            t.checkExpect(new Blob(new Posn(-5, 100), 50, new Red()).outsideBounds(100, 110), true,
                "test outsideBounds above")
            &&

            t.checkExpect(new Blob(new Posn(80, -5), 50, new Blue()).outsideBounds(100, 90), true,
    ...
    boolean testOnMouseClicked(Tester t) {
        return t.checkExpect(this.b1w.onMouseClicked(new Posn(50, 50)), this.b1mouse50x50w);
    }

    /** test the method nearCenter in the Blob class */
    boolean testNearCenter(Tester t) {
        return t.checkExpect(this.b1.nearCenter(200, 200), true, "test nearCenter - true")
            && t.checkExpect(this.b1.nearCenter(200, 100), false, "test nearCenter - false");
    }

    /** the method randomInt in the Blob class */
    boolean testRandomInt(Tester t) {
        return t.checkOneOf("test randomInt", this.b1.randomInt(3), -3, -2, -1, 0, 1, 2, 3)
    ...
    boolean testRandomMove(Tester t) {
        return t.checkOneOf("test randomMove", this.b1.randomMove(1), new Blob(new Posn(99, 99),
            50, new Red()), new Blob(new Posn(99, 100), 50, new Red()), new Blob(new Posn(99, 101),
            50, new Red()), new Blob(new Posn(100, 99), 50, new Red()), new Blob(
            new Posn(100, 100), 50, new Red()), new Blob(new Posn(100, 101), 50, new Red()),
    ...
    boolean testOnTick2(Tester t) {
        return
        // to test the world ending, verify the value of the lastWorld
        t.checkExpect(this.bwOutOfBounds.testOnTick().lastWorld, new WorldEnd(true,
            this.bwOutOfBounds.lastImage("Blob is outside the bounds")))
            &&

            t.checkExpect(this.bwInTheCenter.testOnTick().lastWorld, new WorldEnd(true,
                new OverlayImages(this.bwInTheCenter.makeImage(), new TextImage(new Posn(100, 40),
    ...
    boolean testWorldEnds(Tester t) {
        return t.checkExpect(this.bwOutOfBounds.worldEnds(), new WorldEnd(true, new OverlayImages(
            this.bwOutOfBounds.makeImage(), new TextImage(new Posn(100, 40),
                "Blob is outside the bounds", Color.red))))
            &&

            t.checkExpect(this.bwInTheCenter.worldEnds(), new WorldEnd(true, new OverlayImages(
                this.bwInTheCenter.makeImage(), new TextImage(new Posn(100, 40),
                    "Black hole ate the blob", 13, 3, Color.red)))) &&

            t.checkExpect(this.b1w.worldEnds(), new WorldEnd(false, this.b1w.makeImage()));
    }

    /** run the animation */
    BlobWorldFun w1 = new BlobWorldFun(new Blob(new Posn(100, 200), 20, new Red()));
    BlobWorldFun w2 = new BlobWorldFun(new Blob(new Posn(100, 200), 20, new Red()));
    BlobWorldFun w3 = new BlobWorldFun(new Blob(new Posn(100, 200), 20, new Red()));

    // run the first game
    boolean runAnimation = this.w1.bigBang(200, 300, 0.3);

    /*
     * (just to check that several worlds can run at the same time) boolean
